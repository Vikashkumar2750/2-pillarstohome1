...
Because this app uses a custom Node.js backend and a MySQL database, **you cannot host the entire application on a standard Hostinger Shared Web Hosting plan** (which is designed primarily for PHP and static files). 
...
3. Install MySQL Server:
   ...
   ```
4. Secure MySQL and create a database:
   ```bash
   mysql_secure_installation
   mysql -u root -p
   CREATE DATABASE pillarstohome;
   CREATE USER 'appuser'@'localhost' IDENTIFIED BY 'your_password';
   GRANT ALL PRIVILEGES ON pillarstohome.* TO 'appuser'@'localhost';
   FLUSH PRIVILEGES;
   EXIT;
   ```
5. Install PM2 (a process manager to keep your app running in the background):
   ```bash
   npm install -g pm2
   ```
...
- `.env` (create this on the server with your database credentials)
...
2. Install production dependencies:
   ...
   npm install
   ```
3. Start the backend server using PM2 and `tsx`:
   ```bash
...
If you only have **Hostinger Shared Hosting**, you can host the React frontend there, but you **must** host the Node.js backend (`server.ts`) and MySQL database on a separate service like Render, Railway, or a VPS.

### Step 1: Deploy the Backend and Database
1. Deploy your Node.js backend and a MySQL database to a service like [Render.com](https://render.com) or [Railway.app](https://railway.app).
